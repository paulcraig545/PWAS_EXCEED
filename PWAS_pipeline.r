...
if(!is.null(opt$binary_weights)){
  ...
  binary_protS_outfile <- file.path(out_dir, paste0(cohort, "_binary_PS.rds"))
  ...
  ggsave(paste0(out_dir, cohort, "_", binary_weights_name, "_ps_overalldist.png"), PS_dist_binary, width = 8, height = 6, device='png', dpi=300)
  ggsave(paste0(out_dir, cohort, "_", binary_weights_name, "_ps_overalldensity.png"), PS_density_binary, width = 8, height = 6, device='png', dpi=300)
}
...
if(!is.null(opt$continuous_weights)){
  ...
  continuous_protS_outfile <- file.path(out_dir, paste0(cohort, "_continuous_PS.rds"))
  ...
  ggsave(paste0(out_dir, cohort, "_", continuous_weights_name, "_ps_overalldist.png"), PS_dist_continuous, width = 8, height = 6, device='png', dpi=300)
  ggsave(paste0(out_dir, cohort, "_", continuous_weights_name, "_ps_overalldensity.png"), PS_density_continuous, width = 8, height = 6, device='png', dpi=300)
}
...
if(!is.null(opt$binary_pheno)){
  ...
  if(!is.null(opt$binary_weights)){
    ...
    ggsave(paste0(out_dir, cohort, "_", binary_pheno_name, "_", binary_weights_name, "_dist.png"), binary_pheno_binary_PS_dists, width = 8, height = 6, device='png', dpi=300)
    ggsave(paste0(out_dir, cohort, "_", binary_pheno_name, "_", binary_weights_name, "_density.png"), binary_pheno_binary_PS_densities, width = 8, height = 6, device='png', dpi=300)

    binary_pheno_binary_PS_covs <- merge(binary_pheno_binary_PS, all_covs, by = id_col)
    
    binary_PS_formula <- as.formula(
        paste0("as.factor(pheno) ~ scale(binary_protS) + ", paste(covs_formu, collapse = " + "))
    )
    
    binary_pheno_binary_PS_assoc_mod <- glm(binary_PS_formula, 
    ...
    binary_pheno_binary_PS_assoc_coefs_outfile <- file.path(out_dir, paste0(cohort, "_", binary_pheno_name, "_", binary_weights_name, "_PS_assoc_coefs.rds"))
    ...
    print(paste0("AUC value for binary phenotype binary weights: ", auc_value))
    
    # save ROC curve object for plotting all cohorts together
    saveRDS(roc_curve, file.path(out_dir, paste0(cohort, "_", binary_pheno_name, "_", binary_weights_name, "_roc_curve.rds")))
    
    script_name <- "binary_predict"
    # ROC Graph 
    cairo_pdf(file = file.path(out_dir, paste0(cohort, "_", binary_pheno_name, "_", binary_weights_name, "_binary_pheno_binary_PS_assoc_ROC_curve.pdf")), width = 8, height = 6)
    ...
    cairo_pdf(file = file.path(out_dir, paste0(cohort, "_", binary_pheno_name, "_", binary_weights_name, "_PS_assoc_precision_recall.pdf")), width = 8, height = 6)
  ...
  if(!is.null(opt$continuous_weights)){
    ...
    ggsave(paste0(out_dir, cohort, "_", binary_pheno_name, "_", continuous_weights_name, "_dist.png"), binary_pheno_continuous_PS_dists, width = 8, height = 6, device='png', dpi=300)
    ggsave(paste0(out_dir, cohort, "_", binary_pheno_name, "_", continuous_weights_name, "_density.png"), binary_pheno_continuous_PS_densities, width = 8, height = 6, device='png', dpi=300)

    binary_pheno_continuous_PS_covs <- merge(binary_pheno_continuous_PS, all_covs, by = id_col)
    
    continuous_PS_formula <- as.formula(
        paste0("as.factor(pheno) ~ scale(continuous_protS) + ", paste(covs_formu, collapse = " + "))
    )
    
    binary_pheno_continuous_PS_assoc_mod <- glm(continuous_PS_formula, 
                    family=binomial (link=logit), 
    ...
    # save the coefficients 
    binary_pheno_continuous_PS_assoc_coefs_outfile <- file.path(out_dir, paste0(cohort, "_", binary_pheno_name, "_", continuous_weights_name, "_PS_assoc_coefs.rds"))
    ...
    print(paste0("AUC value for binary phenotype continuous weights: ", auc_value))
    
    # save ROC curve object for plotting all cohorts together
    saveRDS(roc_curve, file.path(out_dir, paste0(cohort, "_", binary_pheno_name, "_", continuous_weights_name, "_roc_curve.rds")))
    
    script_name <- "continuous_predict"
    # ROC Graph 
    cairo_pdf(file = file.path(out_dir, paste0(cohort, "_", binary_pheno_name, "_", continuous_weights_name, "_binary_pheno_continuous_PS_assoc_ROC_curve.pdf")), width = 8, height = 6)
    ...
    cairo_pdf(file = file.path(out_dir, paste0(cohort, "_", binary_pheno_name, "_", continuous_weights_name, "_PS_assoc_precision_recall.pdf")), width = 8, height = 6)
...
if(!is.null(opt$continuous_pheno)){
  ...
  if(!is.null(opt$binary_weights)){
    ...
    ggsave(paste0(out_dir, cohort, "_", continuous_pheno_name, "_", binary_weights_name, "_scatter.png"), continuous_pheno_binary_PS_scatter, width = 8, height = 6, device='png', dpi=300)
    ...
    # save the coefficients 
    continuous_pheno_binary_PS_assoc_coefs_outfile <- file.path(out_dir, paste0(cohort, "_", continuous_pheno_name, "_", binary_weights_name, "_PS_assoc_coefs.rds"))
    ...
    png(filename=file.path(out_dir, paste0(cohort, "_", continuous_pheno_name, "_", binary_weights_name, "_PS_scatter_.rds")))
  ...
  if(!is.null(opt$continuous_weights)){
    ...
    ggsave(paste0(out_dir, cohort, "_", continuous_pheno_name, "_", continuous_weights_name, "_scatter.png"), continuous_pheno_continuous_PS_scatter, width = 8, height = 6, device='png', dpi=300)
    ...
    # save the coefficients 
    continuous_pheno_continuous_PS_assoc_coefs_outfile <- file.path(out_dir, paste0(cohort, "_", continuous_pheno_name, "_", continuous_weights_name, "_PS_assoc_coefs.rds"))
    ...
    png(filename=file.path(out_dir, paste0(cohort, "_", continuous_pheno_name, "_", continuous_weights_name, "_PS_scatter_.rds")))
